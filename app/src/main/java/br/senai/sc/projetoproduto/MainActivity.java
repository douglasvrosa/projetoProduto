...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
        ...
        }else if (resultCode == RESULT_CODE_PRODUTO_EXCLUIR) {
            Produto produtoExcluir = (Produto) data.getExtras().getSerializable("produtoExcluir");
            for (int i = 0; i < adapterProdutos.getCount(); i++) {
                Produto produto = adapterProdutos.getItem(i);
                if (produto.getId() == produtoExcluir.getId()) {
                    adapterProdutos.remove(produto);
                    break;
                }
            }
